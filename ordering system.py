...
class Order:
    ...
    def thy_table(self):
        """
        Mit while True wird die Sequenz wiederholt bis eine Valide Zahl eingegeben wird. Wenn das Geschieht verlässt man
        die Schleife. Alls fail safe haben wir ein except ValueError eingebaut. Verhindert, dass das Programm abschmiert,
        wenn man keine Zahl eingibt.
        """
        while True:
            try:
                ...
                if people <= 0: #Weniger als ein Gast zu bedienen ist halt auch schwer
                   ...
                   return people #Die (hoffentlich) Valide Zahl wird zurückgegeben.
                    ...
                    self.menu.menu_display() #Aufrufen des Menüs, wenn eine Valide Zahl eingegeben wurde. Import aus
                                             # menu.py
                    break #Verässt die Schleif
            except ValueError:
                print("Oh my, what happened there. Lets try it again as if nothing happened, alright?") #Erwähnter fail
                                                                                                        # safe
