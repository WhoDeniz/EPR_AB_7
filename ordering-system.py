...
class Order:
    '''
    Order class contains the order
    '''

    def __init__(self, people: int, menu_file):
        self.people = people
        self.menu = Menu(menu_file)

# Funktoniert noch nicht ganz muss noch daran arbeiten. Ziel ist das wenn nach
# einem Getränk gefragt wird auch nur ein Getränk ausgewählt
# werden kann und das selbe bei den Speisen. Wird also noch gemacht aber ist bereits ein anfang

    def ordering(self):
        '''
        ordering takes the order of the people and displays the menu
        '''
        while self.people <= 0:
